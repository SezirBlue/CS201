...
int main()
{
	while (true)
	{
		...
		{
			dataKeys.push_back(i);
		}
		vector <int> evenNum;
		for (auto n : dataKeys)
		{
			evenNum.push_back(n * 2);
		}
		...
		int keyFind = 1234567;
		int evenFind = keyFind*2;

		// Do searches
		binarySearch(dataKeys, evenNum, keyFind,evenFind);
		cout << "Binary Search DONE" << endl;
		sequentialSearch(dataKeys, evenNum, keyFind,evenFind);
